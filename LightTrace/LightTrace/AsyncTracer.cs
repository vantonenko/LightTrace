...
namespace LightTrace
{
    public sealed class CallStackTrace : IDisposable
    {
        ...
        private static AsyncContext _rootCalls;
        private static AsyncLocal<AsyncContext> _context;
        ...
        {
            var context = _context.Value ?? _rootCalls;
            _context.Value = context.Calls.GetOrAdd(name ?? string.Empty, fName => new AsyncContext(fName, context));
            _stopwatch = Stopwatch.StartNew();
        }

        public static CallContext GetCalls()
        {
            return _rootCalls.GetCallContext();
        }
        ...
        {
            _rootCalls = new AsyncContext("Root", null);
            _context = new AsyncLocal<AsyncContext>();
        }
        ...
        {
            _stopwatch.Stop();
            _context.Value.AddTime(_stopwatch.Elapsed);
            _context.Value = _context.Value.Parent;
        }
